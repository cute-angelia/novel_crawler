...
// 控制爬取流程
func DoCrawler(urlStr, fileName, nums string) (respBookName string, respBookChapterNumTotal int) {
	if c, err := crawler.CreateCrawler(urlStr); err == nil {
		log.Println("正在获取章节列表......")
		if bookName, chapters, err := c.FetchChapterList(); err == nil {
			...
			// 章节范围
			chapterRange := utils.RangeConvert(nums)
			...
			if rf, ok := consts.NewSiteInfoConfigMap[c.GetUrl().Hostname()]; ok {
				...
				chapters = crawler.ExtractRange(chapters, chapterRange)
			...
			respBookChapterNumTotal = len(chapters)
